function [s]=V4_Osintseva_filtrB(y,k,l,M)%функция фильтра
L=length(y);%длина вектора L равна длине y
for i=1:L;%i меняется от 1 до L
    d(i)=y(i);
     if (i>l) %случай, когда i больше k
       d(i)=y(i)-y(i-l);%значение d, когда i больше k
          if (i>k) %случай, когда i больше l
          d(i)=y(i)-y(i-l)-y(i-k);%значение d, когда i больше l
